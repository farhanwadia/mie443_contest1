...
void laserCallback(const sensor_msgs::LaserScan::ConstPtr& msg){
    ...
    ROS_INFO("Min Laser Distance: %f \n Left: %f \n Right: %f", minLaserDist, minLSLaserDist, minRSLaserDist);
}
...
float chooseAngular(float laserSideThreshold, float probSpinToLarger){
    // Chooses the angular velocity and direction
    float prob = randBetween(0.0, 1.0), angular_vel = M_PI/8;
    if(fabs(minLSLaserDist - minRSLaserDist) > laserSideThreshold){
        //If one side's laser distance > other side by more than laserSideThreshold, go to that side
        if(fabs(minLSLaserDist) > fabs(minRSLaserDist)){
            ROS_INFO("LS >> RS. Spin CCW");
            angular_vel = randBetween(0.0, M_PI/8); //improves gmapping resolution compared to always using pi/8
        }
        else{
            ROS_INFO("RS >> LS. Spin CW");
            angular_vel = randBetween(-M_PI/8, 0.0);
        }
    }
    else{
        ...
        if (minLSLaserDist > minRSLaserDist && prob < probSpinToLarger){
            angular_vel = randBetween(0.0, M_PI/8); 
        }
        else{
            angular_vel = randBetween(-M_PI/8, 0.0);
        }
...
void bumperPressedAction(geometry_msgs::Twist* pVel, ros::Publisher* pVel_pub, uint64_t* pSecondsElapsed, 
    ...
    if (any_bumper_pressed && *pSecondsElapsed < 900){
        ...
        if (bumper[CENTER]){
            ROS_INFO("Center hit. Move back and spin 30");
            moveThruDistance(-0.25, 0.2, posX, posY, pVel, pVel_pub, pSecondsElapsed, start, pLoop_rate);
            rotateThruAngle(copysign(M_PI, chooseAngular(0.15, 1.0)), yaw, minLaserDist, true, pVel, pVel_pub, pSecondsElapsed, start, pLoop_rate);
        }
        else if (bumper[LEFT]){
            ROS_INFO("Left hit. Move back and spin 90 CW");
            moveThruDistance(-0.25, 0.2, posX, posY, pVel, pVel_pub, pSecondsElapsed, start, pLoop_rate);
            rotateThruAngle(DEG2RAD(-90), yaw, minLaserDist, true, pVel, pVel_pub, pSecondsElapsed, start, pLoop_rate);
        }
...
void moveAfterStuck(float startX, float startY, geometry_msgs::Twist* pVel, ros::Publisher* pVel_pub, uint64_t* pSecondsElapsed, 
    ...
    if (minLaserDist < 0.45 || minLaserDist > 7){
        moveThruDistance(-0.25, 0.1, posX, posY, pVel, pVel_pub, pSecondsElapsed, start, pLoop_rate);
    }
    else{
        moveThruDistance(0.25, 0.1, posX, posY, pVel, pVel_pub, pSecondsElapsed, start, pLoop_rate);
    }
...
int main(int argc, char **argv){
    ...
    float bestAngle = 0, maxLaserThreshold = 7, clearPathThreshold = 0.75, stopThreshold = 0.6, prob = 1, laserSideThreshold = 0.15;
    int clearPathIters = 0;
    ...
    while(ros::ok() && secondsElapsed <= 900) {
        ros::spinOnce();
        ROS_INFO("Postion: (%f, %f) Yaw: %f deg minLaserDist: %f IMU %f", posX, posY, RAD2DEG(yaw), minLaserDist, yaw_imu);
        ...
        stopThreshold = randBetween(0.6, 0.7); //Adds randomness in determining when to stop and turn
        ...
        if (!any_bumper_pressed && minLaserDist < maxLaserThreshold){
            if (minLaserDist > clearPathThreshold){
                ...
                angular = 0;
                clearPathIters ++;
                if (clearPathIters > 25){
                    rotateThruAngle(copysign(M_PI/6, chooseAngular(laserSideThreshold, 0.6)), yaw, minLaserDist, false, &vel, &vel_pub, &secondsElapsed, start, &loop_rate);
                    clearPathIters = 0;
                }
            }
                ...
                linear = 0.1;
                angular = chooseAngular(laserSideThreshold, 1.0);  
                clearPathIters = 0;   
            }
            else{
                ROS_INFO("Small distance. Rotating.");
                rotateThruAngle(copysign(M_PI, chooseAngular(laserSideThreshold, 1.0)), yaw, minLaserDist, true, &vel, &vel_pub, &secondsElapsed, start, &loop_rate);
                clearPathIters = 0;
/*                 if (prob < 0.6){
            ...
            //moveThruDistance(-0.1, 0.1, posX, posY, &vel, &vel_pub, &secondsElapsed, start, &loop_rate);
            //angular = chooseAngular(laserSideThreshold, 0.825);
            clearPathIters = 0;
        }
